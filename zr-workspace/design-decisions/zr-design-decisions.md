...
> Version: 2.0 | Date: 2026-03-05 | Author: 余博 (Maleekai) · CPO
> Source: OKX全量分析 (74 flows, 429 screenshots) + 8份像素级design-spec + **Binance抽样分析 (42 flows, 310 screenshots)**
...
| DA-013 | 行情列表迷你K线 (24h趋势线, 灰色, 40×20px) | **Binance Markets** | 行情列表 | 比纯数字更直观展示趋势，Binance独有设计 |
| DA-014 | 资产列表行内快捷按钮 (Earn/Trade, 圆角pill按钮) | **Binance Assets** | 资产页 | 减少操作跳转，提升Earn转化率 |
| DA-015 | 提现方式Bottom Sheet (4选项: 站内/链上/法币/OTC卡片式) | **Binance Assets** | 提现入口 | 清晰展示多渠道选择，减少用户困惑 |
| DA-016 | 定投历史模拟器 (回测收益曲线+时间滑块) | **Binance Recurring** | 定投功能 | 可视化历史回测帮助用户理解定投价值 |
...
| IO-002 | 认知伴侣内容 | OKX的教育内容与交易页分离；**Binance有AI Hub但偏推荐(合规风险)** | 在交易确认页显示"相似行情下的历史表现"、"你的30天胜率"；参考Binance Opportunity三Tab架构(市场/日历/持仓)但不做方向性推荐 | 辅助决策而非阻碍决策 |
...
| IO-008 | 情绪感知通知 | OKX通知纯信息型；**Binance有Fear & Greed仪表盘+AI摘要但未绑定交易流程** | 大幅波动时附加"当前市场恐慌指数: 28 (极度恐惧)"；参考Binance仪表盘样式(半圆gauge+数值)集成到首页卡片 | 帮助用户理性判断，不跟风操作 |
| IO-009 | **多资产热力图** | **Binance有加密热力图但无多资产；OKX无此功能** | 按资产类别(加密/股票/外汇/商品)分区的Treemap热力图，色块大小=市值，颜色=涨跌幅 | 快速扫描多市场整体状态，ZR多资产优势可视化 |
| IO-010 | **交易者画像分析** | **Binance有Assets Report画像但嵌入产品推广；OKX无此功能** | Portfolio页自动生成交易风格标签(保守/平衡/激进) + 风险分布饼图 + 中性建议(不推荐产品) | 用户自我认知提升，增强平台粘性 |
